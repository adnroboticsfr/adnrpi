#!/bin/bash
# ADNRPi Pad X11 rotation script
# Rotates display and touch input 180 degrees and disables screen blanking,
# ONLY when the ADNRPi Pad built-in screen is detected (800x480 + touchscreen).
...
/usr/local/bin/adnrpipad-detect.sh --screen-only || exit 0
...
for _ in $(seq 1 10); do
    if /usr/local/bin/adnrpipad-detect.sh; then
        DETECTED=1
        break
    fi
    sleep 1
...
echo "ADNRPi Pad rotation applied"
